...
wandb.init(project="publishers-classifier-trainer", entity="raya-abu-ahmad")
...
class ExtraBertClassifier(PreTrainedModel):
    ...
    def __init__(self, config_class):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(self.config.hidden_dim + self.config.kg_dim + self.config.publisher_dim, self.config.mlp_dim),
            nn.ReLU(),
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, class_kge, publisher_emb, labels):
        ...
        concat_output = torch.cat((text_embeddings, kg_embeddings, publisher_emb), dim=1)
...
def prepare_dataset(document_text, class_kges, publisher_emb, class_text, labels):
...
def main():
    ...
    # Prepare dataset
    tokenized_dataset = prepare_dataset(document_text, class_kges, publisher_emb, class_text, labels)
